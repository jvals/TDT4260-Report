...
prefetcher. SDP keeps a table of entries indexed by the current load
address. The first time a load is found, the address is stored in a
new entry, together with the address of the data. If the load address
is encountered again, the difference between the current data address
and the previous data address is calculated. The result, called the
``stride'', is added to the current data address and the resulting address
is finally prefetched.
